...
Phase 2 partner-routing records use explicit item prefixes in the same on-demand projects table:

- `PARTNER#{partner_id}` stores `Partners`: `partner_id`, `firm_name`, `assigned_cpa_name`, `cpa_license_number`, `state_jurisdictions`, and `contracted_rev_share_percentage`.
- `ROUTING#{project_id}` stores `ProjectRouting`: `source_project_id`, `assigned_partner_id`, `routing_status`, `partner_notes`, and `routing_event_id`.
- `PAYOUT#{project_id}#{ledger_id}` stores `PayoutLedger`: total client fee collected, platform share, partner share, and `partner_fee_type = SubcontractedProfessionalServiceFees`.
- `AUDIT#{project_id}#{audit_entry_id}` stores immutable `AuditLedger` entries for routing, review, requested changes, and partner sign-off.

Partner review payloads are generated by `PartnerPortalService` as black-box review packets. They include project facts, sanitized report sections, source citations, and allowed review actions. They intentionally exclude LLM prompts, embedding data, vector index details, API tokens, and provider metadata.
